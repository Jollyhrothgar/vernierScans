...
class BeamPositionSteps
{
  ...
  public:
   ...
   unsigned int GetNumberOfSteps(); /** returns size of the vernierScanBPMSteps object */
   time_t GetTimeOffset(); /** returns the time corresponding to the first entry of the data file. Use to sync with WcmDcct Data */
   time_t GetScanStartTime();
   time_t GetScanEndTime();
   time_t GetCentralStepTime(int step_i ); /** Returns the epoch time closest to the
               center of the step# */
   // Estimate BPM accuracy by looking at RMS of fill, corrected for beam movement

   /*** PRDF ANALYSIS ***/ 
   int LookupStep(double lookup_time); 
   /** takes an epoch time stamp and determines which scan step it belongs to. 
    * Double is used for half-second precision in cases where we need to average betwee
    * time stamps. */
   ...
   */
    ...
    * is in the first scan sweep */
   bool IsSecondScan(time_t time_index);
   /** These functions will determine, from the steps files, whether or not a time index
    * is in the second scan sweep */
  ...
  private:
   ...
   // private nested struct to hide implementation of data organization
   // to keep namespace clear and to focus user on public interface
   // of this class.
   struct BeamPosition
   {
     float blx7;
     ...
     void Print() {
       std::cout << " " << time 
           ...
           << " " << hSeparation << " " << vSeparation << std::endl; 
     }
   };

   // private nested class to hide organization/implementation of BPM step data.
   struct BeamStep
   {
     BeamStep() {
       bpmUncertainty = 10.;
   ...
   /* holds the beginning and end of each scan step. Add GetTimeOffset() to 
    * get the epoch time of the step. steps stores relative time to start of 
    * BPM data file for the run (not the whole fill). This complicated crap
    * is because otherwise, its hard to tell where in the scan we are by eye
    * since we'd have an epoch stamp, as opposed to a simple number between 
    * 0 and the number of seconds in the bpm data file for the run 
    *
